...
public class PasswordKeyboardView extends KeyboardView implements KeyboardView.OnKeyboardActionListener {
    ...
    private void init(Context context, AttributeSet attrs, int defStyleAttr) {
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.PasswordKeyboardView,
                defStyleAttr, 0);
        mDeleteDrawable = a.getDrawable(R.styleable.PasswordKeyboardView_deleteDrawable);
        mDeleteBackgroundColor = a.getColor(
                R.styleable.PasswordKeyboardView_deleteBackgroundColor, Color.TRANSPARENT);
        mDeleteWidth = a.getDimensionPixelOffset(R.styleable.PasswordKeyboardView_deleteWidth,
                -1);
        mDeleteHeight = a.getDimensionPixelOffset(R.styleable.PasswordKeyboardView_deleteHeight,
                -1);
        mCompleteTextColor = a.getColor(R.styleable.PasswordKeyboardView_completeTextColor, Color.TRANSPARENT);
        mCompleteTextSize = a.getDimensionPixelOffset(R.styleable.PasswordKeyboardView_completeTextSize, -1);
        a.recycle();
    ...
    public void onDraw(Canvas canvas) {
        ...
        for (Keyboard.Key key : keys) {
            if (key.codes[0] == KEYCODE_EMPTY) {// 如果是左下角空白的按键,重画按键的背景
//                drawKeyBackground(key, canvas, mDeleteBackgroundColor);
//                drawCompleteButton(key, canvas);
                drawCompleteButton(key, canvas, mDeleteBackgroundColor);
            } else if (key.codes[0] == Keyboard.KEYCODE_DELETE) {// 如果是右下角的删除按键,重画按键的背景,并且绘制删除图标
                drawKeyBackground(key, canvas, mDeleteBackgroundColor);
     ...
     * @param canvas
     */
    private void drawCompleteButton(Keyboard.Key key, Canvas canvas, int color) {
        TextDrawable drawable = TextDrawable.builder()
                .beginConfig()
                .textColor(mCompleteTextColor)
                .fontSize(mCompleteTextSize)
                .width(key.width)
                .height(key.height)
                .endConfig().buildRect((String) key.label, color);
        drawable.setBounds(key.x, key.y, key.x + key.width, key.y + key.height);
        drawable.draw(canvas);
    }
    ...
    public void onKey(int primaryCode, int[] keyCodes) {
        if (mOnKeyboardListener != null)
            switch (primaryCode) {
                case Keyboard.KEYCODE_DELETE://点击了删除按键
                    mOnKeyboardListener.onDeleteKeyEvent();
                    break;
                case KEYCODE_EMPTY://点击了完成键
                    mOnKeyboardListener.onCompleteLeyEvent();
                    break;
                default://点击了数字按键
